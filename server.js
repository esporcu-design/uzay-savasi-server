...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    // ── DÜŞMANA VURUŞ BİLDİRİMİ (relay — GÜNCELLEME 39, client "bu düşmana bu kadar hasar verdim" der, sadece HOST işler) ──
    if (msg.type === 'hitEnemy') {
      if (!ws.roomCode) return;
      broadcastToRoom(ws.roomCode, ws, 'peerHitEnemy', {
        playerId: ws.playerId,
        eid: msg.eid,
        dmg: msg.dmg,
      });
    ...
    // ── ORTAK ÖDÜL BİLDİRİMİ (relay — GÜNCELLEME 39, host bir düşman öldüğünde client'a da skor/altın payı yollar) ──
    if (msg.type === 'sharedReward') {
      if (!ws.roomCode) return;
      broadcastToRoom(ws.roomCode, ws, 'peerSharedReward', {
        playerId: ws.playerId,
        score: msg.score,
        gold: msg.gold,
      });
